...
class CustoTnPLeptonProducer_miniAOD : public edm::EDProducer {
public:
  explicit CustoTnPLeptonProducer_miniAOD(const edm::ParameterSet&);
...
CustoTnPLeptonProducer_miniAOD::CustoTnPLeptonProducer_miniAOD(const edm::ParameterSet& cfg)
...
void CustoTnPLeptonProducer_miniAOD::embedTriggerMatch(pat::Muon* new_mu, const std::string& ex, const pat::TriggerObjectStandAloneCollection& L3, std::vector<int>& L3_matched) {
...
void CustoTnPLeptonProducer_miniAOD::embedTriggerMatch_or(pat::Muon* new_mu, const std::string& ex, const pat::TriggerObjectStandAloneCollection& L3, const pat::TriggerObjectStandAloneCollection& L3_or, std::vector<int>& L3_matched, std::vector<int>& L3_matched_2) {
    int best_1 = -1;
    int best_2 = -1;
    float defaultpTvalue = 20.;
...
template <typename T>
edm::OrphanHandle<std::vector<T> > CustoTnPLeptonProducer_miniAOD::doLeptons(edm::Event& event, const edm::InputTag& src, const edm::InputTag& view_src, const std::string& instance_label) {
...
//    std::cout<<"prescaled_L3_muons.size()"<<prescaled_L3_muons.size()<<std::endl;
    ...
    if (electron_muon_veto_dR > 0) {
      if (!muons.isValid())
	throw cms::Exception("CustoTnPLeptonProducer_miniAOD") << "requested to veto electrons near muons, but main muon collection is invalid!\n";
