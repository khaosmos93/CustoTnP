...
#include "CustoTnP/src/DileptonUtilities.h"
#include "CustoTnP/src/PATUtilities.h"
#include "CustoTnP/src/TrackUtilities.h"
#include "CustoTnP/src/ToConcrete.h"
...
class CustoTnPPairSelector_AOD : public edm::EDProducer {
public:
  explicit CustoTnPPairSelector_AOD(const edm::ParameterSet&);
...
CustoTnPPairSelector_AOD::CustoTnPPairSelector_AOD(const edm::ParameterSet& cfg)
...
void CustoTnPPairSelector_AOD::remove_overlap(pat::CompositeCandidateCollection& cands) const {
...
std::vector<reco::TransientTrack> CustoTnPPairSelector_AOD::get_transient_tracks(const pat::CompositeCandidate& dil) const {
...
std::pair<bool, float> CustoTnPPairSelector_AOD::back_to_back_cos_angle(const pat::CompositeCandidate& dil) const {
...
std::pair<bool, CachingVertex<5> > CustoTnPPairSelector_AOD::vertex_constrained_fit(const pat::CompositeCandidate& dil) const {
...
void CustoTnPPairSelector_AOD::embed_vertex_constrained_fit(pat::CompositeCandidate& dil, const CachingVertex<5>& vtx) const {
  if (!vtx.isValid()) {
...
float CustoTnPPairSelector_AOD::dpt_over_pt(const reco::CandidateBaseRef& lep) const {
...
///
std::pair<bool, float> CustoTnPPairSelector_AOD::pt_ratio(const pat::CompositeCandidate& dil) const {
  double pt0 = dil.daughter(0)->pt();
  double pt1 = dil.daughter(1)->pt();

  double the_pt_ratio = pt0 > pt1 ? (pt0/pt1) : (pt1/pt0);

  //if(!ShutUp)  std::cout << "CustoTnPPairSelector_AOD::pt_ratio[" << pt_ratio_max << "] : pt_ratio = " << the_pt_ratio << ", " << (the_pt_ratio < pt_ratio_max) << std::endl;

  return std::make_pair( the_pt_ratio < pt_ratio_max , the_pt_ratio );
}

///
std::pair<bool, float> CustoTnPPairSelector_AOD::dil_deltaR(const pat::CompositeCandidate& dil) const {

  double the_deltaR = reco::deltaR( *dil.daughter(0), *dil.daughter(1) );

  //if(!ShutUp)  std::cout << "CustoTnPPairSelector_AOD::dil_deltaR[" << dil_deltaR_min << "] : deltaR = " << the_deltaR << ", " << (the_deltaR > dil_deltaR_min) << std::endl;

  return std::make_pair( the_deltaR > dil_deltaR_min , the_deltaR );
}

///
float CustoTnPPairSelector_AOD::veto_others_dphi(edm::Event& event, const reco::CandidateBaseRef& lep) {
...
std::vector<int> CustoTnPPairSelector_AOD::countDTsegs(edm::Event& event, reco::MuonRef muon) {
  double DTCut = 30.;
...
std::vector<int> CustoTnPPairSelector_AOD::countCSCsegs(edm::Event& event, reco::MuonRef muon) {
  double CSCCut = 30.;
...
std::pair<bool, reco::MuonRef> CustoTnPPairSelector_AOD::getMuonRef(edm::Event& event, const reco::CandidateBaseRef& lep) {
...
std::vector<int> CustoTnPPairSelector_AOD::nHits(edm::Event& event, reco::MuonRef MuRef) {
...
void CustoTnPPairSelector_AOD::produce(edm::Event& event, const edm::EventSetup& setup) {
  if(!ShutUp)  std::cout << "CustoTnPPairSelector_AOD::produce : Start!" << std::endl;
  ...
  for (pat::CompositeCandidateCollection::const_iterator c = cands->begin(), ce = cands->end(); c != ce; ++c) {
    ...
    //if(!ShutUp)  std::cout << "CustoTnPPairSelector_AOD::produce dpt_over_pt : lep0 dpt_over_pt=" << lep0_dpt_over_pt << ", " << "lep1 dpt_over_pt=" << lep1_dpt_over_pt << std::endl;
    ...
    if(!ShutUp) {
      const pat::Muon* mu0 = toConcretePtr<pat::Muon>(lep0);
      const pat::Muon* mu1 = toConcretePtr<pat::Muon>(lep1);
      std::cout << "CustoTnPPairSelector_AOD::produce : RECO MuonRef" << std::endl;
  ...
  if(!ShutUp && new_cands->size()>0) {
    std::cout << "CustoTnPPairSelector_AOD::produce : TnP Pair found!" << std::endl;
