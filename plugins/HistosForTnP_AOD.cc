...
#include "CustoTnP/src/GeneralUtilities.h"
...
class CustoTnPHistosForTnP_AOD : public edm::EDAnalyzer {
 public:
  explicit CustoTnPHistosForTnP_AOD(const edm::ParameterSet&);
...
CustoTnPHistosForTnP_AOD::CustoTnPHistosForTnP_AOD(const edm::ParameterSet& cfg)
...
void CustoTnPHistosForTnP_AOD::getBSandPV(const edm::Event& event) {
...
std::pair<int, int> CustoTnPHistosForTnP_AOD::getNShowers(const pat::CompositeCandidate& dil) {
  int lep0_nShowers = 0;
...
void CustoTnPHistosForTnP_AOD::fillTnPControlHistos(const pat::CompositeCandidate& dil,
...
void CustoTnPHistosForTnP_AOD::fillTnPBinHistos( double               dil_mass,
                                              double               probe_pt,
                                              bool                 isPassNoPt,
                                              double               binValue,
...
void CustoTnPHistosForTnP_AOD::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  if( !event.isRealData() ){
    ...
    //if(!ShutUp)  std::cout << "CustoTnPHistosForTnP_AOD::analyze : PU = " << thePU << "  PU weight = " << _pileupWeight << std::endl;
  }

  _totalWeight = 1.;
  if( _madgraphWeight != 1. || _pileupWeight != 1. )
    _totalWeight = _madgraphWeight * _pileupWeight;  // prescale weight is not considered yet
  //if(!ShutUp)  std::cout << "CustoTnPHistosForTnP_AOD::analyze : Total weight = " << _totalWeight << std::endl;
  ...
  if( !dileptons.isValid() ) {
    std::cout << "CustoTnPHistosForTnP_AOD::analyze : !dileptons.isValid() ---> return" << std::endl;
    return;
  }
    ...
    if(lep0.isNonnull() && lep1.isNonnull()) {
      ...
      if(mu0 && mu1) {
        ...
        if( tag_selector(*mu0) && (lep0_dpt_over_pt < tag_dpt_over_pt_max) && (fabs(mu0->muonBestTrack()->dz( vertex->position() )) < tag_dz_max) &&
          ...
          if(!ShutUp)  std::cout << "CustoTnPHistosForTnP_AOD::analyze : Tag0 and Probe1" << std::endl;
        ...
        if( tag_selector(*mu1) && (lep1_dpt_over_pt < tag_dpt_over_pt_max) && (fabs(mu1->muonBestTrack()->dz( vertex->position() )) < tag_dz_max) &&
          ...
          if(!ShutUp)  std::cout << "CustoTnPHistosForTnP_AOD::analyze : Tag1 and Probe0" << std::endl;
          if(!ShutUp)  std::cout << "                                              pT=" << ProbeMu->pt() << std::endl;
          if(!ShutUp)  std::cout << "                                             eta=" << ProbeMu->eta() << std::endl;
          if(!ShutUp)  std::cout << "                                             phi=" << ProbeMu->phi() << std::endl;
          if(!ShutUp)  std::cout << "                                            nVtx=" << nVtx << std::endl;
...
DEFINE_FWK_MODULE(CustoTnPHistosForTnP_AOD);
