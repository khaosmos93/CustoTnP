...
class TTbarGenMass : public edm::EDFilter {
 public:
  explicit TTbarGenMass(const edm::ParameterSet&);
...
TTbarGenMass::TTbarGenMass(const edm::ParameterSet& cfg)
...
bool TTbarGenMass::filter(edm::Event& event, const edm::EventSetup&) {
  ...
  //std::cout << "TT mass = "<<massTT << "    " << (massTT >= min_gen && massTT<=max_gen) << std::endl;

  if(isFind1 && isFind2){
    //std::cout << "Fill gen histograms" << std::endl;
    pre_Gen_Weight->Fill(madgraphWeight);
    pre_res_mass->Fill(TT.M(), madgraphWeight);
    pre_res_pt->Fill(TT.Pt(), madgraphWeight);
    //pre_res_eta->Fill(TT.Eta(), madgraphWeight);
    pre_res_phi->Fill(TT.Phi(), madgraphWeight);
    pre_res_rap->Fill(TT.Rapidity(), madgraphWeight);

    if(massTT >= min_gen && massTT<=max_gen){
      Gen_Weight->Fill(madgraphWeight);
      res_mass->Fill(TT.M(), madgraphWeight);
      res_pt->Fill(TT.Pt(), madgraphWeight);
      //res_eta->Fill(TT.Eta(), madgraphWeight);
      res_phi->Fill(TT.Phi(), madgraphWeight);
      res_rap->Fill(TT.Rapidity(), madgraphWeight);
    }
  }
  //std::cout << std::endl;

  return
    isFind1 && isFind2 && massTT >= min_gen && massTT<=max_gen;

}


DEFINE_FWK_MODULE(TTbarGenMass);
