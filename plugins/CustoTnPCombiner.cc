...
struct CustoTnPPairSelector {
  ...
  CustoTnPPairSelector(const std::string& loose_cut,
  ...
  bool operator()(const reco::Candidate& c1, const reco::Candidate& c2) const {
    ...
    //printf("in mumu CustoTnPPairSelector %s with c1 pt %f c2 pt %f  loose1 %i loose2 %i tight1 %i tight2 %i\n", module_label.c_str(), c1.pt(), c2.pt(), loose(c1), loose(c2), tight(c1), tight(c2));
...
namespace reco {
  namespace modules {
    ...
    template<>
    struct ParameterAdapter<CustoTnPPairSelector> {
      static CustoTnPPairSelector make(const edm::ParameterSet& cfg) {
	return CustoTnPPairSelector(cfg.getParameter<std::string>("loose_cut"),
    ...
    typedef CandCombiner<
      ...
      CustoTnPPairSelector,
      combiner::helpers::ShallowClone,
      pat::CompositeCandidateCollection
      > CustoTnPCombiner;

    DEFINE_FWK_MODULE(CustoTnPCombiner);
  } 
}
